#!/usr/bin/env bash
# rendererとHTTP serverをtrainerとは別tmux sessionで起動し、local/public readbackする。
...
readonly MONITOR_ROOT="$EXPERIMENT_ROOT/monitor/$RUN_NAME"
...
mkdir -p "$MONITOR_ROOT" "$GATE_DIR" "$EXPERIMENT_ROOT/logs/monitor"
python3 "$EXPERIMENT_SCRIPT_DIR/monitor.py" \
  --run-name "$RUN_NAME" --run-root "$RUN_ROOT" --output-dir "$MONITOR_ROOT" --once

printf -v render_command 'exec python3 %q --run-name %q --run-root %q --output-dir %q >>%q 2>&1' \
  "$EXPERIMENT_SCRIPT_DIR/monitor.py" "$RUN_NAME" "$RUN_ROOT" "$MONITOR_ROOT" \
  "$EXPERIMENT_ROOT/logs/monitor/render-$RUN_NAME.log"
printf -v http_command 'exec python3 %q --root %q --bind 0.0.0.0 --port %q >>%q 2>&1' \
  "$EXPERIMENT_SCRIPT_DIR/monitor_server.py" "$MONITOR_ROOT" "$MONITOR_PORT" \
...
for _ in $(seq 1 30); do
  if curl --silent --show-error --fail --user "$MONITOR_USER:$MONITOR_PASSWORD" \
    "http://127.0.0.1:$MONITOR_PORT/status.json" >/dev/null; then
    break
  fi
  sleep 1
done
...
unauth_status=$(curl --silent --output /dev/null --write-out '%{http_code}' "http://127.0.0.1:$MONITOR_PORT/")
[[ "$unauth_status" == 401 ]] || fail "monitorの未認証requestが401になりません: $unauth_status"
curl --silent --show-error --fail --user "$MONITOR_USER:$MONITOR_PASSWORD" \
...
echo "[monitor] local/mapped readbackと未認証401を確認しました"
