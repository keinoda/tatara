...
if [[ "$RETRY_STEP" == prepare_data && -e "$TRAIN_PSV.partial" ]]; then
  [[ "${CONFIRM_REMOVE_PARTIAL_PSV:-}" == "$TRAIN_PSV.partial" ]] \
    || fail "partial PSVを消して再試行する場合はCONFIRM_REMOVE_PARTIAL_PSV=$TRAIN_PSV.partialを明示してください"
  rm -- "$TRAIN_PSV.partial"
  echo "[retry] 生成途中のpartial PSVだけを削除しました: $TRAIN_PSV.partial"
fi
